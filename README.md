...
| `LevelCatalog.*` | 외부 레벨 목록의 순서, 경로, 적 수와 스폰 시드 검증 |
...
- 미로 로더, 레벨 목록, 셀 좌표 변환, 스폰과 투사체·구체 연속 충돌 자동 테스트 25개 통과
- 서로 다른 크기의 `Level01`과 `Level02` 전환 및 상태·리소스 재설정 확인
- 적 피격·사망, 플레이어 접촉 게임 오버와 현재 레벨 재시작 확인
- 레벨 1의 적 1마리와 레벨 2의 적 2마리가 시드 기반 위치에 생성되고 처치 후 재생성되지 않음을 확인
...
- 외부 레벨 목록에서 경로, 적 수와 스폰 시드를 읽고 클리어 시 기존 지오메트리와 게임 상태를 정리한 뒤 다음 미로를 재구성합니다.
